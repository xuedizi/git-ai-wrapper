...
- `managed-update.patch` adds the TCLI-owned safe update protocol and daemon startup barrier. See
  [lifecycle protocol and validation](docs/managed-update.md).
...
runs the library suite, enterprise/installer regressions, and managed lifecycle TestRepo tests before native release builds.
